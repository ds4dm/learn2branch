...
def load_batch_gcnn(sample_files):
    ...
    n_cands_per_sample = [cds.shape[0] for cds in cands]
    ...
    cands = np.concatenate([cands + shift
        for cands, shift in zip(cands, cv_shift[1])])
    ...
    cands = tf.convert_to_tensor(cands, dtype=tf.int32)
    ...
    return c_features, e_indices, e_features, v_features, n_cs_per_sample, n_vs_per_sample, n_cands_per_sample, cands, cand_choices, cand_scoress
